...
def main():
    ...
    if driver is None:
        print(
            "Here is a chromedriver issue. Please make sure you installed everything correctly"
        )
        exit()
    ...
    all_text = [
        div.get_text()
        for div in soup.find_all("div", class_="style-scope ytd-channel-name")
    ]
    ...
    print(
        "Are the number of links and names equal?: ", len(all_names) == len(all_links)
    )

    if len(all_names) == 0 or len(all_links) == 0:
        print("Please try again. Sometimes there are issue with the youtube page.")
    subscriptions = [
        {"service_id": 0, "url": f"https://www.youtube.com{link}", "name": name}
        for name, link in list(zip(all_names, all_links))
    ]
